...
@IonicPage()
@Component({
  selector: 'page-sccmptedt5plist',
  templateUrl: 'sccmptedt5plist.html',
})

export class Sccmptedt5plistPage {
  ...
  ngOnInit(val) {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetCompletedT5PendingJobCardList?BranchID=" + this.global.UserDetails[0].BranchID).subscribe(jobCards => {
  ...
  JCListClick(jc) {
    if (!this.global.IsManager) {
      this.navCtrl.setRoot(Sccmptedt5pdetailsPage, { data: jc });
    }
  }

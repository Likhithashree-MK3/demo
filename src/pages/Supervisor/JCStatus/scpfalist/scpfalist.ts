...
@IonicPage()
@Component({
  selector: 'page-scpfalist',
  templateUrl: 'scpfalist.html',
})

export class ScpfalistPage {
  ...
  ngOnInit(val) {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetPendingForAllocationJobCardList?BranchID=" + this.global.UserDetails[0].BranchID).subscribe(jobCards => {
  ...
  JCListClick(jc) {
    if (!this.global.IsManager) {
      this.navCtrl.setRoot(ScpfadetailsPage, { data: jc });
    }
  }

...
@IonicPage()
@Component({
  selector: 'page-sccmptedlist',
  templateUrl: 'sccmptedlist.html',
})

export class SccmptedlistPage {
  ...
  ngOnInit(val) {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetCompletedJobCardList?BranchID=" + this.global.UserDetails[0].BranchID).subscribe(jobCards => {
  ...
  JCListClick(jc) {
    if (!this.global.IsManager) {
      this.navCtrl.setRoot(SccmpteddetailsPage, { data: jc });
    }
  }

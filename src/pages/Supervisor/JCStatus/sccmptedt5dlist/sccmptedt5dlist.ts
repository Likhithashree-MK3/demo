...
@IonicPage()
@Component({
  selector: 'page-sccmptedt5dlist',
  templateUrl: 'sccmptedt5dlist.html',
})

export class Sccmptedt5dlistPage {
  ...
  ngOnInit(val) {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetCompletedT5DoneJobCardList?BranchID=" + this.global.UserDetails[0].BranchID).subscribe(jobCards => {
  ...
  JCListClick(jc) {
    if (!this.global.IsManager) {
      this.navCtrl.setRoot(Sccmptedt5ddetailsPage, { data: jc });
    }
  }

...
@IonicPage()
@Component({
  selector: 'page-scetdelist',
  templateUrl: 'scetdelist.html',
})

export class ScetdelistPage {
  ...
  ngOnInit(val) {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetETDExceededJobCardList?BranchID=" + this.global.UserDetails[0].BranchID + "&JobCardType=T2PFA").subscribe(jobCards => {
  ...
  JCListClick(jc) {
    if (!this.global.IsManager) {
      this.navCtrl.setRoot(ScetdedetailsPage, { data: jc });
    }
  }

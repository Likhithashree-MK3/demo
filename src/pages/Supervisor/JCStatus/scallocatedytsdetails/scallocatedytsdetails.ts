...
import { ScallocatedytslistPage } from '../scallocatedytslist/scallocatedytslist';
...
@IonicPage()
@Component({
  selector: 'page-scallocatedytsdetails',
  templateUrl: 'scallocatedytsdetails.html',
})

export class ScallocatedytsdetailsPage {
    ...
    if (this.global.CheckInternetConnection()) {
      ...
      this.httpClient.get<any>(this.global.HostedPath + "GetJobCardDetails?JobCardID=" + this.SelectedJC.JobCardHedIC).subscribe(jobs => {
        ...
        if (jobs.StatusCode == 200) {
          ...
          this.JobsList = JSON.parse(jobs.Output).GetJCJobDetails;
          this.SelectedSEList = JSON.parse(jobs.Output).GetJobCardTechnicianList;
